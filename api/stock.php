...
try {
    ...
    if (MENU == 'list') {
        ...
        // 2020-11-10 수정 히스토리를 기록
        $row = $db->query("SELECT * FROM smartstore_stock WHERE id={$data['id']}");

        if (empty($row)) {
            throw new Exception(null, 400);
        }

        $addAmount = $data['amount'] - $row[0]['amount'];

        $db->query("INSERT INTO smartstore_stock_history (id, amount) VALUES('{$data['id']}', '{$addAmount}')");
        // 수정 히스토리 기록 끝
        ...
        $data = $_POST['data'];
        $result = $db->query("SELECT * FROM smartstore_stock_history WHERE id={$data['id']}");
    }
